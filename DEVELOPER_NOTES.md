...
CARTO Positron GL style: https://basemaps.cartocdn.com/gl/positron-gl-style/style.json
Replaced the original dark OSM raster tile source.

### Layers and zoom behavior

- plss-fill: section fill colored by landman, opacity 0 where inMonday is false
- plss-outline: section grid lines, #aaaaaa, 0.8px, filtered to inMondayTownship, fades in zoom 8→9
- plss-section-numbers: section numbers from FRSTDIVNO, #333333, no halo, fades in zoom 11→12, filtered to inMondayTownship, leading zeros stripped
- township-borders: township boundary polygons from township-polygons-source, #999999, 1.5px, no filter (covers all Oklahoma townships so the map works as Monday data expands to new areas)
- township-labels: T##N R##W labels from townships.geojson (points), #555555, no halo, fades in zoom 8→9, no fade-out

### Static files in client/public

- four_counties_plss.geojson: PLSS section polygons for the four-county window
- townships.geojson: township centroid points for label placement, generated by scripts/generate-townships.js
- township-polygons.geojson: township boundary polygons for border rendering, generated by scripts/generate-township-polygons.js from four_counties_plss.geojson. Covers all 427 Oklahoma townships in the source data.

### Regenerating township-polygons.geojson

If four_counties_plss.geojson is ever updated, regenerate the township polygons:
  node scripts/generate-township-polygons.js
Then commit the updated township-polygons.geojson to client/public.
